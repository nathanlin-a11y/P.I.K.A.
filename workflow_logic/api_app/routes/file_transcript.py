...
from workflow_logic.core.data_structures import FileType
from workflow_logic.core.agent import PIKAAgent
from workflow_logic.core import PIKAChat, FileReference
...
async def generate_file_transcript(
    ...
    if agent_id:
        ...
        chat: PIKAChat = await db_app.get_chat(chat_id)
        agent = chat.pika_agent if chat else None
        if not agent:
            raise HTTPException(status_code=404, detail="Chat Agent not found")
    else:
        agent = PIKAAgent(name="DefaultTranscriptionAgent")
    ...
    try:
        ...
        await db_app.update_file_reference(file_ref)
