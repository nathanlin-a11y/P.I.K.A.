...
from workflow_logic.core import BasicAgentTask, FunctionParameters, ParameterDefinition, PIKAAgent, Prompt, PIKAModel, TaskResponse, MessageDict, LLMChatOutput, ApiType
...
def sample_agent():
    return PIKAAgent(
        ...
        models= {"chat": PIKAModel(short_name="TestModel", model_name="test-model", model_format="OpenChat", ctx_size=1000, model_type="chat", deployment="test")}
    )
...
async def test_basic_agent_task_initialization(sample_basic_agent_task):
    ...
    assert isinstance(sample_basic_agent_task.agent, PIKAAgent)
...
@pytest.mark.asyncio
@patch.object(PIKAAgent, 'chat', new_callable=AsyncMock)
