...
from workflow_logic.core import PIKAModel, FunctionParameters, ParameterDefinition, SearchOutput, ModelConfig, ApiType, ApiName
...
async def test_generate_response_with_api_engine_llm(api_manager, sample_api):
    ...
    with patch('workflow_logic.core.api.api_manager.get_api_engine', return_value=lambda: mock_llm_engine):
...
async def test_generate_response_with_api_engine_search(api_manager):
    ...
    with patch('workflow_logic.core.api.api_manager.get_api_engine', return_value=lambda: mock_search_engine):
        response = await api_manager.generate_response_with_api_engine(
            api_type=ApiType.GOOGLE_SEARCH,
            query="test query"
        )
