...
from workflow_logic.core.tasks import APITask, CVGenerationTask, PIKATask, RedditSearchTask, Workflow, WikipediaSearchTask, GoogleSearchTask, ExaSearchTask, ArxivSearchTask, BasicAgentTask, PromptAgentTask, CheckTask, CodeGenerationLLMTask, CodeExecutionLLMTask, AgentWithFunctions
...
class BackendAPI(BaseModel):
    ...
    def get_agents(self, agent_id: Optional[str] = None) -> Dict[str, PIKAAgent]:
        if agent_id is None:
    ...
    def task_initializer(self, task: dict) -> PIKATask:
    ...
    def get_tasks(self, task_id: Optional[str] = None) -> Dict[str, PIKATask]:
        if task_id is None:
    ...
    def get_models(self, model_id: Optional[str] = None) -> Dict[str, PIKAModel]:
        if model_id is None:
    ...
    def get_chats(self, chat_id: Optional[str] = None) -> Dict[str, PIKAChat]:
        if chat_id is None:
    ...
    def populate_chat(self, chat: dict) -> PIKAChat:
        print(f'Chat: {chat}')
            ...
            for function_id in chat['functions']:
                try:
                    ...
                    if not isinstance(task, PIKATask):
                        print(f"Warning: task_initializer returned non-PIKATask object for ID {function_id}")
                        continue
        ...
        try:
            chat_obj = PIKAChat.model_validate(chat)
            if not chat_obj:
                raise ValueError(f"Invalid chat object after processing functions. {chat}")
            
            chat_obj.pika_agent.model_manager_object = self.model_manager
            return chat_obj
        except Exception as e:
