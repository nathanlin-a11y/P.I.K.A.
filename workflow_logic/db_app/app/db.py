...
from workflow_logic.core import PIKAAgent, PIKAChat, Prompt, PIKAModel, PIKATask, API
from workflow_logic.util.const import BACKEND_PORT, HOST, ADMIN_TOKEN
from workflow_logic.util import User, DatabaseTaskResponse, MessageDict
...
class BackendAPI(BaseModel):
    ...
    async def populate_chat(self, chat: dict) -> PIKAChat:
            ...
            for function in chat['functions']:
                try:
                    ...
                    if not isinstance(task, PIKATask):
                        print(f"Warning: task_initializer returned non-PIKATask object for function {function.get('task_name', 'Unknown')}")
                        continue
        ...
        try:
            ...
            print(f"Error creating PIKAChat object: {str(e)}")
