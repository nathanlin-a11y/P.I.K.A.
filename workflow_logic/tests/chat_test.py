...
class ChatTests(TestModule):
    ...
    async def run(self, db_init_manager: DBInitManager, **kwargs) -> Dict[str, Any]:
        ...
        for chat in db_init_manager.entity_obj_key_map.get("chats", {}).values():
            # Test each chat
            if not isinstance(chat, PIKAChat):
                if isinstance(chat, dict):
                    chat = PIKAChat(**chat)
                else:
                    raise ValueError(f"Chat object is not of type PIKAChat: {chat}")
    ...
    async def test_chat(self, chat: PIKAChat, api_manager: APIManager) -> Dict[str, str]:
        test_results = {}
