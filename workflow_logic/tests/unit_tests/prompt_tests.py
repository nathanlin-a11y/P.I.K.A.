...
class TestPrompt(unittest.TestCase):
    ...
    def test_format(self):
        prompt = Prompt(**self.prompt_data)
        formatted_content = prompt.format(name="PIKA")
        self.assertEqual(formatted_content, "Hello, PIKA!")

    def test_format_prompt(self):
        prompt = Prompt(**self.prompt_data)
        formatted_content = prompt.format_prompt(name="PIKA")
        self.assertEqual(formatted_content, "Hello, PIKA!")
    ...
    def test_validate_input(self):
        ...
        prompt.validate_input(name="PIKA")
    ...
    def test_partial(self):
        prompt = Prompt(**self.prompt_data)
        partial_prompt = prompt.partial(name="PIKA")
        self.assertEqual(partial_prompt.partial_variables["name"], "PIKA")
        self.assertNotIn("name", partial_prompt.parameters.properties)
        self.assertNotIn("name", partial_prompt.parameters.required)
        self.assertIn("PIKA", partial_prompt.content)
