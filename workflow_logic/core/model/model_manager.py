...
from workflow_logic.core.model.model import PIKAModel

class ModelManager(BaseModel):
    model_definitions: List[dict | PIKAModel] = Field(const_model_definitions, title="Model Definitions", description="The model definitions. Can be passes as dict or PIKAModel objects, after init they will be converted to PIKAModel objects.")
    ...
    def validate_model_definitions(self):
        for model in self.model_definitions:
            if not isinstance(model, PIKAModel):
                try:
                    model = PIKAModel.model_validate(model)
                except ValidationError as e:
                    raise ValidationError(
                        f"Validation error in PIKAModel: {str(e)}",
                        model=model,
                    )
        return self
    ...
    @property
    def default_model(self) -> PIKAModel:
    ...
    @property
    def default_vision_model(self) -> PIKAModel:
    ...
    def get_model_obj_from_short_name(self, short_name: str) -> PIKAModel | None:
    ...
    def get_model_obj_from_name(self, name: str) -> PIKAModel | None:
