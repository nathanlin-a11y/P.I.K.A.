...
class APIManager(BaseModel):
    ...
    Methods:
        ...
        retrieve_api_data(api_type: ApiType, model: Optional[PIKAModel] = None) -> Union[Dict[str, Any], LLMConfig]:
            Retrieves API data or LLMConfig based on the API type and model.

    Private Methods:
        _retrieve_llm_api_data(model: Optional[PIKAModel] = None) -> LLMConfig:
            Retrieves LLMConfig for LLM APIs.
        _create_llm_config(api: API, model: PIKAModel) -> LLMConfig:
    ...
    def _retrieve_llm_api_data(self, model: Optional[PIKAModel] = None) -> LLMConfig:
    ...
    def _create_llm_config(self, api: API, model: PIKAModel) -> LLMConfig:
