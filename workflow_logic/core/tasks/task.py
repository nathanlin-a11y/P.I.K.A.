...
from workflow_logic.util.communication import TaskResponse, DatabaseTaskResponse
...
class PIKATask(BaseModel, ABC):
    """
    An abstract base class representing a task in the PIKA workflow system.

    PIKATask serves as the foundation for all task types in the system. It defines
    ...
    Attributes:
        ...
        tasks (Optional[Dict[str, "PIKATask"]]): Subtasks that this task may use.
        ...
        agent (Optional[PIKAAgent]): The agent associated with this task.
        execution_agent (Optional[PIKAAgent]): The agent responsible for executing this task.
    ...
    The PIKATask class provides a robust framework for defining complex, multi-step
    ...
    tasks: Optional[Dict[str, "PIKATask"]] = Field(default={}, description="A dictionary of task_id: task")
    ...
    agent: Optional[PIKAAgent] = Field(None, description="The agent that the task is associated with")
    execution_agent: Optional[PIKAAgent] = Field(None, description="The agent that the task is executed by")
    ...
    def model_dump(self, *args, **kwargs):
        ...
        if 'tasks' in data and isinstance(data['tasks'], dict):
            data['tasks'] = {
                task_id: task.model_dump(*args, **kwargs) if isinstance(task, PIKATask) else task
        ...
        # Handle other potential nested PIKATask objects and enums
        for key, value in dumped_data.items():
            if isinstance(value, PIKATask):
                dumped_data[key] = value.model_dump(*args, **kwargs)
            elif isinstance(value, list):
                dumped_data[key] = [
                    item.value if isinstance(item, Enum) else
                    item.model_dump(*args, **kwargs) if isinstance(item, PIKATask) else item
                    for item in value
                ]
            elif isinstance(value, dict):
                dumped_data[key] = {
                    k: (v.value if isinstance(v, Enum) else
                        v.model_dump(*args, **kwargs) if isinstance(v, PIKATask) else v)
