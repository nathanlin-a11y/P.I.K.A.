...
class TaskLibrary(BaseModel):
    available_tasks: List[PIKATask] = Field([], description="A list of tasks to make available in the library on initialization")
    agent_library: AgentLibrary = Field(..., description="A library of agents available for task execution")
    tasks: Dict[str, PIKATask] = Field({}, description="A dictionary of tasks available in the library")
    ...
    def add_task(self, task: PIKATask) -> bool:
        if not isinstance(task, PIKATask):
            logging.warning(f"Task {task} is not an instance of PIKATask. Skipping.")
            return False
    ...
    def get_task(self, task_name: str) -> PIKATask:
    ...
    def add_tasks_in_batch(self, tasks: List[PIKATask]) -> bool:
