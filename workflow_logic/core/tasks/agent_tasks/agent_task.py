...
class BasicAgentTask(PIKATask):
    ...
    async def run(self, api_manager: APIManager, **kwargs) -> TaskResponse:     
        new_messages, exitcode, start_messages = await self.generate_agent_response(api_manager, **kwargs)
        exec_history = kwargs.pop("execution_history", None)
        if not new_messages or not new_messages.messages: 
            return self.get_failed_task_response(diagnostics="No messages generated by the agent", **kwargs)
        # Log the type of new messages
        LOGGER.info(f'new_messages: {new_messages.messages}')
        LOGGER.info(f'new_messages type: {type(new_messages.messages[0])}')
        output_str = new_messages.detailed_summary()
