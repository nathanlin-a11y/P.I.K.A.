...
from workflow_logic.core.model import ModelManager, PIKAModel
...
class PIKAAgent(BaseModel):
    ...
    model: Optional[PIKAModel] = Field(default=None, description="The model object for the agent")
...
class AgentLibrary(BaseModel):
    agents: Dict[str, PIKAAgent] = Field({}, description="A dictionary of agents with their names as keys")
    model_manager_object: ModelManager = Field(description="The model manager object")
    model_config = ConfigDict(protected_namespaces=())

    def get_agent_by_name(self, agent_name: str) -> PIKAAgent:
        if agent_name not in self.agents:
            raise ValueError(f"Agent {agent_name} not found in the agent library.")
        return self.agents[agent_name]

    def add_agent(self, agent: PIKAAgent) -> bool:
    ...
    def get_agent_by_id(self, agent_id: str) -> PIKAAgent:
