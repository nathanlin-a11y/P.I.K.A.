...
export const chatHelpers = {
    async create_chat(chat: Partial<IPIKAChatDocument>, userId?: string): Promise<IPIKAChatDocument | null> {
        try {
    ...
    async edit_chat(chatId: string, chat: Partial<IPIKAChatDocument>, userId?: string): Promise<IPIKAChatDocument | null> {
        try {
            const originalChat = await PIKAChat.findById(chatId);
            if (!originalChat) {
            ...
            chatHelpers.checkAndUpdateChanges(originalChat, newChatData, changeHistoryData, 'pika_agent');
            ...
            const updatedChat = await PIKAChat.findByIdAndUpdate(
                chatId,
                {
                    ...newChatData,
    ...
    async edit_message_in_chat(chatId: string, msgId: string, message: Partial<IMessage>, userId?: string): Promise<IMessage | null> {
        try {
            const updatedChat = await PIKAChat.findOneAndUpdate(
    ...
    async remove_message_in_chat(chatId: string, msgId: string, userId?: string): Promise<null> {
        try {
            await PIKAChat.findByIdAndUpdate(
                chatId,
                {
    ...
    async add_task_result_to_chat(chatId: string, taskResultId: string, userId?: string): Promise<IMessage | null> {
        try {
            ...
            if (messageToRemoveId) {
              await PIKAChat.findByIdAndUpdate(chatId, {
