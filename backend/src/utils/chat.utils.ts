...
export async function createChat(
    chatData: Partial<IPIKAChatDocument>,
    userId: string
): Promise<IPIKAChatDocument | null> {
    try {
        ...
        const chat = new PIKAChat(chatData);
        await chat.save();
        ...
        if (messages && Array.isArray(messages)) {
            ...
            return await PIKAChat.findByIdAndUpdate(
                chat._id,
                { messages: messageIds },
                { new: true }
            );
...
export async function updateChat(
    ...
    try {
        ...
        // Check for changes in 'pika_agent' and 'functions'
        checkAndUpdateChanges(originalChat, chatData, changeHistoryData, 'pika_agent');
...
export async function createMessageInChat(
    ...
    try {
        ...
        const populatedChat = await popService.findAndPopulate(PIKAChat, chatId, userId);
