...
const userDefaultChatConfigSchema = new Schema({
  pika_agent: { type: Schema.Types.ObjectId, ref: 'Agent', required: true },
...
function ensureObjectIdForSave(
  ...
  if (this.default_chat_config) {
    if (this.default_chat_config.pika_agent) {
      this.default_chat_config.pika_agent = getObjectId(this.default_chat_config.pika_agent);
    }
...
function ensureObjectIdForUpdate(
  ...
  if (update.default_chat_config) {
    if (update.default_chat_config.pika_agent) {
      update.default_chat_config.pika_agent = getObjectId(update.default_chat_config.pika_agent);
    }
...
function autoPopulate(this: mongoose.Query<any, any>) {
  this.populate('default_chat_config.pika_agent')
...
userSchema.methods.apiRepresentation = function(this: IUserDocument) {
  const defaultChatConfig = this.default_chat_config ? {
    pika_agent: this.default_chat_config.pika_agent ? 
      (this.default_chat_config.pika_agent._id || this.default_chat_config.pika_agent) : null,
