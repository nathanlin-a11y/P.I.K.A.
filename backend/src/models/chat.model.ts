...
function ensureObjectIdForSave(
  ...
  const context = { model: 'PIKAChat', field: '' };
  if (this.pika_agent) this.pika_agent = getObjectId(this.pika_agent, { ...context, field: 'pika_agent' });
...
function ensureObjectIdForUpdate(
  ...
  const context = { model: 'PIKAChat', field: '' };
  if (update.pika_agent) update.pika_agent = getObjectId(update.pika_agent, { ...context, field: 'pika_agent' });
...
pikaChatSchema.plugin(mongooseAutopopulate);
