...
import { MessageType, PIKAChat, PIKAAgent } from '../utils/types';
...
const ChatPIKA: React.FC = () => {
  ...
  const handleSelectAgent = async (agentId: string | null): Promise<PIKAAgent> => {
    if (agentId) {
  ...
  const handleSelectExecutorAgent = async (agentId: string | null): Promise<PIKAAgent> => {
    if (agentId) {
      const agent = await fetchItem("agents", agentId);
      return agent;
    } else {
  ...
  const handleSelectChat = async (chat: PIKAChat | null) => {
    if (chat) {
      try {
        ...
        setAgents([chat.pika_agent]);
        console.log('Agents:', [chat.pika_agent]);  // Add this log
      } catch (error) {
      ...
      try {
        ...
        const newChat = await createChat({ name: 'New Chat', pika_agent: agent._id, executor: executor_agent._id }); // Replace with actual logic for creating a new chat
