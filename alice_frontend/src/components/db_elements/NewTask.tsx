...
import { PIKAAgent, Prompt, PIKATask, PromptAgentTaskForm, CheckTaskForm, CodeGenerationLLMTaskForm, CodeExecutionLLMTaskForm, AgentWithFunctionsForm, WorkflowForm, TaskType } from '../../utils/types';
...
interface NewTaskProps {
  taskId?: string;
  viewOnly?: boolean;
  onTaskCreated?: (task: PIKATask) => void;
}
...
const NewTask: React.FC<NewTaskProps> = ({ taskId, viewOnly = false, onTaskCreated }) => {
  ...
  const castToPIKATask = (form: any, selectedTaskType: string): Partial<PIKATask> => {
    if (!isValidTaskType(selectedTaskType)) {
      throw new Error(`Invalid task type: ${selectedTaskType}`);
    }
    const baseTask: Partial<PIKATask> = {
  ...
  const handleSave = async () => {
    try {
      const taskData = castToPIKATask(taskForm, selectedTaskType);
      if (taskId) {
        const savedTask = await updateTask(taskId, taskData);
        if (savedTask && onTaskCreated) onTaskCreated(savedTask as PIKATask)
      } else {
