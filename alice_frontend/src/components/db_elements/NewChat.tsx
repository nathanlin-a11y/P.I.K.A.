...
import { PIKAAgent, PIKATask, PIKAModel, LLMConfig, CreatePIKAChat } from '../../utils/types';
...
interface NewChatProps {
  open: boolean;
  onClose: () => void;
  onChatCreated: (chat: CreatePIKAChat) => void;
}
...
const NewChat: React.FC<NewChatProps> = ({ open, onClose, onChatCreated }) => {
  ...
  const [viewingAgent, setViewingAgent] = useState<PIKAAgent | null>(null);
  const [viewingModel, setViewingModel] = useState<PIKAModel | null>(null);
  const [viewingFunction, setViewingFunction] = useState<PIKATask | null>(null);
  ...
  const handleCreateChat = async () => {
    ...
    const newChat: CreatePIKAChat = {
      name: chatName,
      pika_agent: selectedAgent,
