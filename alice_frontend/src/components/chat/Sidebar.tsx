...
import { PIKAChat, PIKAAgent, TaskResponse, PIKATask } from '../../utils/types';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [selectedAgent, setSelectedAgent] = useState<PIKAAgent | null>(null);
  const [selectedFunction, setSelectedFunction] = useState<PIKATask | null>(null);
  ...
  const handleAgentClick = (agent: PIKAAgent) => {
    setSelectedAgent(agent);
    setOpenAgentDialog(true);
  };

  const handleFunctionClick = (func: PIKATask) => {
          ...
          <AccordionDetails className={classes.accordionDetails}>
            {currentChat && (
              <>
                ...
                <List>
                  <ListItemButton onClick={() => currentChat.pika_agent && handleAgentClick(currentChat.pika_agent)}>
                    <ListItemIcon><SupportAgent /></ListItemIcon>
                    <ListItemText 
                      primary="PIKA Agent"
                      secondary={currentChat.pika_agent?.name || 'N/A'}
                    />
                  </ListItemButton>
