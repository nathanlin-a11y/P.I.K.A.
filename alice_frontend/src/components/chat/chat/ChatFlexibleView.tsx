...
const ChatFlexibleView: React.FC<ChatComponentProps> = ({ 
    ...
    const handleModelChange = (selectedModel: Partial<PIKAModel>) => {
        updateChatField('model_id', selectedModel);
    };

    const handleAgentChange = (selectedAgent: Partial<PIKAAgent>) => {
        updateChatField('pika_agent', selectedAgent);
    };

    const handleExecutorChange = (selectedAgent: Partial<PIKAAgent>) => {
        updateChatField('executor', selectedAgent);
    };

    const handleTaskChange = (selectedTask: Partial<PIKATask>) => {
        const updatedFunctions = item.functions 
            ? item.functions.some(f => f._id === selectedTask._id)
                ...
                : [...item.functions, selectedTask as PIKATask]
            : [selectedTask as PIKATask];
