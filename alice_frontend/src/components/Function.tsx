...
import { PIKATask, PIKAAgent, Prompt, FunctionParameters } from '../utils/types';
...
const Function: React.FC<FunctionProps> = ({ functionId, onClose, viewOnly = false }) => {
  ...
  const [task, setTask] = useState<Partial<PIKATask>>({
    task_name: '',
    task_description: '',
  ...
  const [allTasks, setAllTasks] = useState<PIKATask[]>([]);
  ...
  const handleMapChange = (mapName: keyof PIKATask, key: string, value: string | number) => {
    if (viewOnly) return;
    setTask((prev) => {
  ...
  const handleArrayChange = (arrayName: keyof PIKATask, value: any[]) => {
    if (viewOnly) return;
