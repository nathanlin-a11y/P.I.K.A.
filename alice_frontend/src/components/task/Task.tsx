...
import { PIKATask, TaskType, convertToPIKATask } from '../../utils/TaskTypes';
...
type BaseTaskMode = BaseDbElementProps<PIKATask>['mode'];
...
const EnhancedTask: React.FC<EnhancedTaskProps> = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setAgents(await fetchItem('agents') as PIKAAgent[]);
            setPrompts(await fetchItem('prompts') as Prompt[]);
            setAvailableTasks(await fetchItem('tasks') as PIKATask[]);
        };
        fetchData();
    }, [fetchItem]);
    ...
    const TaskForm: React.FC<{
        task: PIKATask | null;
        onChange: (newTask: Partial<PIKATask>) => void;
        ...
        const renderTaskForm = () => {
            ...
            const getTaskComponent = <T extends Partial<PIKATask>>(
    ...
    const renderTask = useCallback((
        ...
        const pika_task = convertToPIKATask(task);
        return <TaskForm task={pika_task} onChange={onChange} mode={mode} handleSave={handleSave} />;
    }, [TaskForm]);
    ...
    const renderViewMode = (
        ...
        const executeTask = async (inputs: any) => {
            ...
            setSelectedTask(item as PIKATask)
            setInputValues(inputs)
            await handleExecuteTask()
            return selectedResult
        };
        const renderSingleItem = (task: PIKATask) => {
            switch (props.mode) {
                case 'list':
                case 'shortList':
                    return (
        ...
        if (props.fetchAll && items) {
            return (
                <List>
                    {items.map((task) => (
                        <Box key={task._id}>
                            {renderSingleItem(convertToPIKATask(task))}
                        </Box>
                    ))}
                </List>
            );
        } else if (item) {
            return renderSingleItem(convertToPIKATask(item));
        } else {
