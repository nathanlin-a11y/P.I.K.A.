...
from workflow.core import PIKAModel, PIKAChat, APIManager, TaskResponse, MessageDict, ApiType
...
class ChatTests(TestModule):
    ...
    async def simulate_api_response(api_manager: APIManager, api_type: ApiType, model: Optional[PIKAModel] = None, **kwargs):
    ...
    async def test_function_execution(self, chat: PIKAChat, api_manager: APIManager) -> str:
        ...
        try:
            ...
            async def simulate_api_response(api_type: ApiType, model: Optional[PIKAModel] = None, **kwargs):
                return MessageDict(
                    role="assistant",
                    content="I'm calling a function to help with your request.",
                    tool_calls=[{
            ...
            with patch.object(APIManager, 'generate_response_with_api_engine', new=simulate_api_response), \
                patch.object(PIKAChat, 'tool_map', return_value=mock_tool_map):
    ...
    async def test_code_execution(self, chat: PIKAChat, api_manager: APIManager) -> str:
        try:
            # Mock code execution
            with patch('workflow.core.agent.PIKAAgent._execute_code_in_docker', new_callable=AsyncMock) as mock_execute:
