...
from workflow.db_app import DBInitManager
from workflow.core import APIManager, PIKATask, TaskResponse
...
class TaskTests(TestModule):
    ...
    def test_task_initialization(self, tasks: Dict[str, PIKATask]) -> str:
        try:
            for task_id, task in tasks.items():
                if not isinstance(task, PIKATask):
                    return f"Task {task_id} is not an instance of PIKATask"
            return "Success"
        except Exception as e:
            return f"Error in task initialization: {str(e)}"

    async def test_task(self, task: PIKATask, api_manager: APIManager) -> Dict[str, str]:
        test_results = {}
    ...
    async def test_basic_execution(self, task: PIKATask, api_manager: APIManager) -> str:
        try:
    ...
    def test_api_validation(self, task: PIKATask, api_manager: APIManager) -> str:
        try:
    ...
    def test_function_generation(self, task: PIKATask) -> str:
        try:
    ...
    async def test_recursion_handling(self, task: PIKATask, api_manager: APIManager) -> str:
        try:
