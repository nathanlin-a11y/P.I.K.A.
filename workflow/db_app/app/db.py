...
from workflow.core import PIKAAgent, PIKAChat, Prompt, PIKAModel, PIKATask, API, User, MessageDict, FileReference, FileContentReference
...
class BackendAPI(BaseModel):
    ...
    Attributes:
        ...
        available_task_types (list[PIKATask]): List of available task types.
    ...
    Methods:
        ...
        get_agents(agent: Optional[str] = None) -> Dict[str, PIKAAgent]: Retrieves agents.
        get_tasks(task_id: Optional[str] = None) -> Dict[str, PIKATask]: Retrieves tasks.
        get_models(model_id: Optional[str] = None) -> Dict[str, PIKAModel]: Retrieves models.
        ...
        get_chats(chat_id: Optional[str] = None) -> Dict[str, PIKAChat]: Retrieves chats.
        store_chat_message(chat_id: str, message: MessageDict) -> PIKAChat: Stores a chat message.
    ...
    available_task_types: list[PIKATask] = Field(available_task_types, frozen=True, description="The available task types")
    ...
    @property
    def task_types(self) -> Dict[str, PIKATask]:
    ...
    async def get_agents(self, agent: Optional[str] = None) -> Dict[str, PIKAAgent]:
        if agent is None:
            ...
            try:
                async with session.get(url, headers=headers) as response:
                    ...
                    if isinstance(agents, list):
                        ...
                        return {agent["_id"]: PIKAAgent(**agent) for agent in preprocessed_agents}
                    else:
                        preprocessed_agent = await self.preprocess_data(agents)
                        return {agents["_id"]: PIKAAgent(**preprocessed_agent)}
            except aiohttp.ClientError as e:
                LOGGER.error(f"Error retrieving agents: {e}")
                return {}

    async def get_tasks(self, task_id: Optional[str] = None) -> Dict[str, PIKATask]:
        if task_id is None:
    ...
    async def get_models(self, model_id: Optional[str] = None) -> Dict[str, PIKAModel]:
        if model_id is None:
            ...
            try:
                async with session.get(url, headers=headers) as response:
                    ...
                    if isinstance(models, list):
                        ...
                        return {model['_id']: PIKAModel(**model) for model in models}
                    else:
                        models = await self.preprocess_data(models)
                        return {models['_id']: PIKAModel(**models)}
            except aiohttp.ClientError as e:
    ...
    async def task_initializer(self, task: dict) -> PIKATask:
    ...
    async def get_chats(self, chat_id: Optional[str] = None) -> Dict[str, PIKAChat]:
        if chat_id is None:
            ...
            try:
                async with session.get(url, headers=headers) as response:
                    ...
                    if isinstance(chats, list):
                        ...
                        return {chat["_id"]: PIKAChat(**chat) for chat in chats}
                    else:
                        chats = await self.preprocess_data(chats)
                        return {chats['_id']: PIKAChat(**chats)}
            except aiohttp.ClientError as e:
                LOGGER.error(f"Error retrieving chats: {e}")
                return {}
        
    async def populate_chat(self, chat: dict) -> PIKAChat:
        if 'functions' in chat and chat['functions']:
            processed_functions = []
            for function in chat['functions']:
                try:
                    task = await self.task_initializer(function)
                    if not isinstance(task, PIKATask):
                        LOGGER.warning(f"Warning: task_initializer returned non-PIKATask object for function {function.get('task_name', 'Unknown')}")
                        continue
        ...
        try:
            return PIKAChat(**chat)
        except Exception as e:
            LOGGER.error(f"Error creating PIKAChat object: {str(e)}")
            # Might want to add more detailed error handling here
            raise
    
    async def store_chat_message(self, chat_id: str, message: MessageDict) -> PIKAChat:
