...
from workflow.core.tasks import PIKATask, create_task_from_json
...
class PIKAChat(BaseDataStructure):
    ...
    async def continue_user_interaction(self, api_manager: APIManager, interaction: UserInteraction) -> Optional[MessageDict]:
        ...
        try:
            ...
            if next_step == CheckpointType.TOOL_CALL and target_message.references.tool_calls:
                ...
                code_blocks = self.pika_agent.collect_code_blocks([target_message])
                if code_blocks:
    ...
    def _can_tool_call(self, message: MessageDict) -> Union[bool, UserInteraction]:
        ...
        if not message.references or not message.references.tool_calls or self.pika_agent.has_tools == 0  or self.pika_agent.has_tools == 3: # Disabled or dry run
            return False
    ...
    def _can_execute_code(self, message: MessageDict) -> Union[bool, UserInteraction]:
        ...
        code_blocks = self.pika_agent.collect_code_blocks([message])
        if not code_blocks or not message.references or not message.references.code_executions or self.pika_agent.has_code_exec == 0: # Disabled
            return False
    ...
    async def _handle_tool_calls(self, api_manager: APIManager, tool_calls: List[Dict]) -> List[TaskResponse]:
        ...
        # Execute tool calls
        messages: List[MessageDict] = await self.pika_agent.process_tool_calls(
            tool_calls,
    ...
    def available_tools(self) -> Optional[List[PIKATask]]:
