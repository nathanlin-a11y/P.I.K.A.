...
class PIKAChat(BaseDataStructure):
    """
    ...
    Attributes:
        ...
        agent_tools (Optional[List[PIKATask]]): List of available tools/tasks for the agent.
    ...
    agent_tools: Optional[List[PIKATask]] = Field(default_factory=list, description="List of tools to be registered with the agent")
    retrieval_tools: Optional[List[PIKATask]] = Field(default_factory=list, description="Optional retrieval task for accessing data cluster")
    ...
    def initialize_tools(self) -> 'PIKAChat':
        """
        Ensures agent_tools and retrieval_tools are properly initialized as lists of PIKATask instances.
        
        This validator:
        1. Converts None to empty list for agent_tools
        2. Ensures retrieval_tools is either None or a list
        3. Casts dictionary items to PIKATask instances
        4. Validates that all items are or can be converted to PIKATask instances
        """
        ...
        # Convert agent_tools items to PIKATask instances
        processed_agent_tools = []
        for tool in self.agent_tools:
            if isinstance(tool, dict):
                try:
                    processed_agent_tools.append(PIKATask.model_validate(tool))
                except Exception as e:
                    raise ValueError(f"Failed to convert dictionary to PIKATask: {str(e)}")
            elif isinstance(tool, PIKATask):
                processed_agent_tools.append(tool)
            else:
                raise ValueError(f"Agent tools must be either dictionaries or PIKATask instances. Found: {type(tool)}")
        ...
        if self.retrieval_tools is not None:
            ...
            # Convert retrieval_tools items to PIKATask instances
            processed_retrieval_tools = []
            for tool in self.retrieval_tools:
                if isinstance(tool, dict):
                    try:
                        processed_retrieval_tools.append(PIKATask.model_validate(tool))
                    except Exception as e:
                        raise ValueError(f"Failed to convert dictionary to PIKATask: {str(e)}")
                elif isinstance(tool, PIKATask):
                    processed_retrieval_tools.append(tool)
                else:
                    raise ValueError(f"Retrieval tools must be either dictionaries or PIKATask instances. Found: {type(tool)}")
    ...
    def model_dump(self, *args, **kwargs):
        """
        Serializes the PIKAChat instance to a dictionary, ensuring proper handling of:
        ...
        Returns:
            dict: The serialized PIKAChat instance
        """
        ...
        if self.pika_agent and isinstance(self.pika_agent, BaseModel):
    ...
    async def _execute_single_turn(self, api_manager: APIManager, previous_messages: List[MessageDict], user_data: Optional[User] = None) -> List[MessageDict]:
        ...
        try:
            ...
            if self.pika_agent.has_code_exec:
    ...
    async def _handle_tool_calls(self, api_manager: APIManager, tool_calls: List[Dict], parent_message: MessageDict) -> List[MessageDict]:
        ...
        # Execute tool calls
        return await self.pika_agent.process_tool_calls(
            tool_calls,
            self.get_available_tools(api_manager),
    ...
    async def _handle_code_execution(self, api_manager: APIManager, messages: List[MessageDict]) -> List[MessageDict]:
        ...
        if self.pika_agent.has_code_exec == 1:  # DISABLED
            return []
        ...
        if self.pika_agent.has_code_exec == 2:  # WITH_PERMISSION
            # Check for existing interaction
        ...
        # Execute code
        executed_messages, code_by_lang, exit_code = await self.pika_agent.process_code_execution(messages)
