...
class PIKAChat(BaseDataStructure):
    ...
    pika_agent: PIKAAgent = Field(
        default=PIKAAgent(
            ...
            system_message=Prompt(
                name="pika_default",
                content="You are PIKA, an AI personal assistant powered by a suite of tools."
            ),
        ),
        description="The PIKA agent for this chat session"
    )
    agent_tools: List[PIKATask] = Field(
        default_factory=list, 
        description="Available tools for the agent"
    )
    retrieval_tools: List[PIKATask] = Field(
        default_factory=list, 
    ...
    def initialize_tools(self) -> 'PIKAChat':
        ...
        # Convert dictionaries to PIKATask instances
        self.agent_tools = [
            PIKATask.model_validate(tool) if isinstance(tool, dict) else tool
    ...
    def model_dump(self, *args, **kwargs):
        ...
        if isinstance(self.pika_agent, BaseModel):
    ...
    async def _execute_single_turn(self, api_manager: APIManager, previous_messages: List[MessageDict], user_data: Optional[User] = None) -> List[MessageDict]:
        ...
        try:
            ...
            # Handle tool calls
            if llm_message.references.tool_calls and self.pika_agent.has_tools:
    ...
    async def _handle_tool_calls(self, api_manager: APIManager, tool_calls: List[Dict], skip_permission: bool = False) -> List[MessageDict]:
        ...
        if not skip_permission and self.pika_agent.has_tools == 2:  # WITH_PERMISSION
    ...
    async def _handle_code_execution(self, messages: List[MessageDict], skip_permission: bool = False) -> List[MessageDict]:
        ...
        if self.pika_agent.has_code_exec == 0:  # DISABLED
            return []
        ...
        if not skip_permission and self.pika_agent.has_code_exec == 2:  # WITH_PERMISSION
            # Create new checkpoint
            checkpoint = self.default_user_checkpoints[CheckpointType.CODE_EXECUTION]
            return [self._create_checkpoint_message(checkpoint)]

        # Execute code
        code_executions, exit_code = await self.pika_agent.process_code_execution(messages)
        return [MessageDict(
    ...
    async def _generate_llm_response(self, api_manager: APIManager, messages: List[MessageDict], tools_list: List[ToolFunction], **kwargs) -> MessageDict:
        ...
        return await self.pika_agent.generate_llm_response(
            api_manager, 
            messages, 
            tools_list, 
            **kwargs
