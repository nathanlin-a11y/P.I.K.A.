...
from workflow.core.data_structures.task_response_new import TaskResponse, NodeResponse
from workflow.core.data_structures.user_interaction import UserInteraction
from workflow.util import LOGGER
from workflow.core.tasks.task_new import PIKATask
...
class Workflow(PIKATask):
    ...
    This class inherits from PIKATask, allowing it to be used as part of another workflow.
    It manages the execution flow of multiple tasks, handling task selection, execution,
    and result processing.

    Attributes:
        tasks (Dict[str, PIKATask]): A dictionary of tasks in the workflow.
    ...
    async def run(self, **kwargs) -> TaskResponse:
        ...
        if not execution_history:
            initial_task_name, _ = self.get_initial_task()
        else:
