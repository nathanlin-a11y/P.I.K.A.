...
const ChatPIKA: React.FC = () => {
  ...
  const selectChatId = useCallback(async (chat: PIKAChat | PopulatedPIKAChat) => {
  ...
  const checkAndAddTask = useCallback((task: PIKATask | PopulatedTask) => {
      ...
      <Box className={classes.chatPIKAMain}>
        <Box className={classes.chatPIKAMessages}>
          {currentChatId && currentChat ? !currentThread ? (
            <PlaceholderSkeleton
              mode="chat"
              text='Select a thread (or start a new one) to start chatting with PIKA.'
            />
          ) : (
            <ChatThreadFullView />
          ) : (
...
export default React.memo(ChatPIKA);