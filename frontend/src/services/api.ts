...
import { PIKAChat, convertToPIKAChat, convertToPopulatedPIKAChat, PopulatedPIKAChat } from '../types/ChatTypes';
...
export const resumeChat = async (interaction: UserInteraction): Promise<PIKAChat> => {
  try {
    ...
    // Set up WebSocket connection to receive updates
    const updatedChat = await new Promise<PIKAChat>((resolve, reject) => {
      setupWebSocketConnection(taskId, async (message: any) => {
        if (message.status === 'completed') {
          const chatId = interaction.owner.id;
          const updatedChatData = await fetchItem('chats', chatId) as PIKAChat;
          resolve(convertToPIKAChat(updatedChatData));
        } else if (message.status === 'failed') {
...
export const sendMessage = async (chatId: string, message: PopulatedMessage): Promise<PopulatedPIKAChat> => {
  try {
    ...
    return convertToPopulatedPIKAChat(response.data.chat);
  } catch (error) {
