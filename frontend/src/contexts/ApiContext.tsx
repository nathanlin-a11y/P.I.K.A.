...
export const ApiProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const generateChatResponse = useCallback(async (chatId: string, threadId: string): Promise<boolean> => {
        try {
            ...
            if (result) {
                ...
                const updatedChat = await apiFetchPopulatedItem('chats', chatId) as PopulatedPIKAChat;
    ...
    const addThreadToChat = useCallback(async (chatId: string, threadId: string): Promise<PIKAChat> => {
        try {
    ...
    const removeThreadFromChat = useCallback(async (chatId: string, threadId: string): Promise<PIKAChat> => {
        try {
    ...
    const resumeChat = useCallback(async (interaction: UserInteraction): Promise<PIKAChat> => {
        try {
    ...
    const updateMessageInChat = useCallback(async (chatId: string, message: MessageType): Promise<MessageType> => {
        try {
            ...
            const updatedChat = await apiFetchItem('chats', chatId) as PIKAChat;
            emitEvent('updated', 'chats', updatedChat);
            return result;
        } catch (error) {
