export interface BaseBlock {
    type: 'markdown' | 'pikaDocumentCustomBlock' | 'analysisCustomBlock';
    content: string;
    data?: {
        hName: string;
        hProperties: {
...
export const parseMarkdownBlocks = (content: string, blockTypes: string[]): BaseBlock[] => {
    ...
    while (currentIndex < content.length) {
        ...
        for (const blockType of blockTypes) {
            ...
            if (blockType === 'pikaDocument') {
                const docMatch = content.slice(currentIndex).match(/^<pikaDocument\s+([^>]*)>/);
                if (docMatch && docMatch.index === 0) {
                    const docStart = currentIndex;
                    const docTagEnd = currentIndex + docMatch[0].length;
                    const docEnd = content.indexOf('</pikaDocument>', docTagEnd);
                    ...
                    if (docEnd !== -1) {
                        ...
                        blocks.push({
                            type: 'pikaDocumentCustomBlock',
                            content: content.slice(docTagEnd, docEnd).trim(),
                            data: {
                                hName: 'div',
                                hProperties: {
                                    className: ['custom-block', 'custom-block-pikaDocument']
                                },
                                attributes: attrs
                            }
                        });
                    
                        currentIndex = docEnd + 16; // Corrected length of '</pikaDocument>'
                        lastNormalTextIndex = currentIndex;
                        matchedBlock = true;
                        break; // Exit the for-loop
                    }
