...
const PersonalInformation: React.FC<PersonalInformationProps> = ({
    ...
    const handleAgentToolsChange = useCallback(async (selectedIds: string[]) => {
        const tools = await Promise.all(selectedIds.map(id => fetchItem('tasks', id) as Promise<PIKATask>));
        setUserObject(prevUser => {
            if (!prevUser) return null;
            return {
                ...prevUser,
        ...
        <Card className={classes.card}>
            <CardContent>
                ...
                <EnhancedSelect<PIKAAgent>
                    ...
                    selectedItems={userObject.default_chat_config?.pika_agent ? [userObject.default_chat_config.pika_agent] : []}
