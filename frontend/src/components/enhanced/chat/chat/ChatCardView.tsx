...
import { ChatComponentProps, PopulatedPIKAChat } from '../../../../types/ChatTypes';
...
const ChatCardView: React.FC<ChatComponentProps> = ({
  ...
  const populatedItem = item as PopulatedPIKAChat
  ...
  const listItems = [
    {
      ...
      secondary_text: (
        <ListItemButton onClick={() => populatedItem.pika_agent?._id && selectCardItem && selectCardItem('Agent', populatedItem.pika_agent._id, populatedItem.pika_agent)}>
          {populatedItem.pika_agent?.name || 'N/A'}
        </ListItemButton>
      )
    },
    {
  ...
  return (
    <>
      <CommonCardView
        ...
        listItems={listItems}
        item={item as PopulatedPIKAChat}
        itemType='chats'
      />
    </>
  );
