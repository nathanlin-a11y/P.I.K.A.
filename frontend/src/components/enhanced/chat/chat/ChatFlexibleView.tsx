...
const ChatFlexibleView: React.FC<ChatComponentProps> = ({
    ...
    const [form, setForm] = useState<Partial<PopulatedPIKAChat>>(item as PopulatedPIKAChat || getDefaultChatForm());
        ...
        const populateConfig = async () => {
            if (mode === 'create' && user?.default_chat_config && !item) {
                ...
                const [agent, agentTools, retrievalTools, toolCallCheckpoint, codeExecCheckpoint, dataCluster] = await Promise.all([
                    config.pika_agent ? fetchPopulatedItem('agents', config.pika_agent) : undefined,
                ...
                setForm(item as PopulatedPIKAChat);
    ...
    const handleAgentChange = useCallback(async (selectedIds: PIKAAgent[]) => {
        if (selectedIds.length > 0) {
            setForm(prevForm => ({ ...prevForm, pika_agent: selectedIds[0] }));
        } else {
    ...
    ), [form.pika_agent, handleAgentChange, isEditMode]);
    ...
    return (
        <GenericFlexibleView
            ...
            mode={mode}
            item={form as PopulatedPIKAChat}
            itemType='chats'
        >
            {validationError && (
