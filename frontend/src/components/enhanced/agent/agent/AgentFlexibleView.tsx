...
import { PIKAModel, ModelType } from '../../../../types/ModelTypes';
...
const AgentFlexibleView: React.FC<AgentComponentProps> = ({
    ...
    const [form, setForm] = useState<Partial<PIKAAgent>>(item || getDefaultAgentForm());
    ...
    const handleModelChange = useCallback(async (selectedIds: string[]) => {
        const updatedModels: { [key in ModelType]?: PIKAModel } = {};
        for (const id of selectedIds) {
            const model = await fetchPopulatedItem('models', id) as PIKAModel;
